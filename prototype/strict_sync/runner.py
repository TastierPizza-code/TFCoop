...
async def host(args, secret, *, expected_manifest=None, capabilities=MODEL_CAPABILITIES,
    ...
    def publish(state, reason=""):
        if progress:
            progress(state, reason=reason, round=coordinator.round, frame=coordinator.frame,
                     ...
                     live=coordinator.live_progress() if hasattr(coordinator, "live_progress") else None,
    ...
    try:
        ...
        live_progress = coordinator.live_progress() if hasattr(coordinator, "live_progress") else {}
        observed_progress = live_progress if live_progress.get("started") else stream_progress
        historical = bool(observed_progress.get("started"))
        write_report(args.report, {"backend": backend, "halted": coordinator.halted,
                                   ...
                                   "state_digest_frame": (observed_progress.get("last_observed_frame") if historical
                                                          else coordinator.frame),
                                   "state_digest_sim_time_us": (observed_progress.get("last_observed_time_us") if historical
