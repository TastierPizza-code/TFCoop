"""Bounded launcher-input experiment with input seals on existing replies.

This separate experiment adds bounded launcher depot requests to the accepted
paced schedule. Every depot gets a fresh read-only engine preview after all
...
ID = "manual-depot-input-v1"
MANUAL_DEPOT_CAPABILITY = "manual_depot_v1"
MAX_CYCLES = 2048
...
MANUAL_DEPOT_WORLD_RECEIPTS = frozenset(("live_ready", "live_checkpointed", "live_applied", "live_held", "live_finished", "live_previewed", "live_rejected"))
...
def _need(condition, reason):
    if not condition:
        raise ProtocolError("manual depot probe: " + reason)
...
def schedule():
    return {"id": ID, "step_us": STEP_US, "chunk_steps": CHUNK_STEPS,
            ...
            "input_ops": ["SET_PAUSED", "BUILD_DEPOT", "END_TEST"],
            "depot_order": "after_pause_requests_then_peer_then_sequence",
            "depot_validation": "fresh_read_only_preview_before_each_apply",
            "depot_sites": [1, 2, 3, 4], "depot_rotations": [0, 90, 180, 270]}
...
def _ordered(requests):
    ...
    pauses = sorted((item for item in items if item["command"]["op"] == "SET_PAUSED"),
                    key=lambda item: (item["command"]["value"], item["peer"], item["seq"]))
    depots = sorted((item for item in items if item["command"]["op"] == "BUILD_DEPOT"),
                    key=lambda item: (item["peer"], item["seq"]))
    return pauses + depots
...
class _LiveState:
    def _init_live(self):
        ...
        self._counts = {peer: {"pause": 0, "resume": 0, "end": 0, "depot": 0} for peer in ROSTER}
        ...
        self._depot_preview = None
        self._depot_previews, self._depot_records = [], []
    ...
    def _accept_batch(self, requests, boundary):
        ...
        self._batch_cursor = 0
        pause_order = [item for item in self._batch_order if item["command"]["op"] == "SET_PAUSED"]
        values = {item["command"]["value"] for item in pause_order}
        cross_peer_conflict = any(a["command"]["value"] != b["command"]["value"]
            for a in pause_order if a["peer"] == "a"
            for b in pause_order if b["peer"] == "b")
        if cross_peer_conflict:
            self._conflicts += 1
        for peer in ROSTER:
            for item in requests[peer]:
                op = item["command"]
                name = ("end" if op["op"] == "END_TEST" else "depot" if op["op"] == "BUILD_DEPOT"
                        else "pause" if op["value"] else "resume")
    ...
    def _applied_outcome(self, request, before, after, receipt):
        if request["command"]["op"] == "BUILD_DEPOT":
            outcome = {**copy.deepcopy(request), "status": "applied", "changed_pause": False,
                       "frame": after["frame"], "sim_time_us": after["sim_time_us"],
                       "receipt_digest": digest(receipt), "cost": receipt["result"]["cost"]}
            record = {"request": copy.deepcopy(request), "before": copy.deepcopy(before),
                      "after": copy.deepcopy(after), "receipt": copy.deepcopy(receipt),
                      "preview": copy.deepcopy(self._depot_preview), "batch_hash": self._batch_hash}
            self._command_records.append(record)
            self._depot_records.append(copy.deepcopy(record))
    ...
    def _record_preview(self, request):
        self._depot_previews.append({"request": copy.deepcopy(request),
            "boundary": copy.deepcopy(self._last_world), "preview": copy.deepcopy(self._depot_preview),
            "batch_hash": self._batch_hash})

    def _rejected_outcome(self, request):
        _need(self._depot_preview is not None and self._depot_preview["allowed"] is False,
              "rejection lacks its agreed negative preview")
        outcome = {**copy.deepcopy(request), "status": "rejected",
                   "reason": self._depot_preview["reason"], "cost": 0,
                   "frame": self._last_world["frame"], "sim_time_us": self._last_world["sim_time_us"],
                   "preview_digest": digest(self._depot_preview)}
        self._depot_records.append({"request": copy.deepcopy(request), "before": copy.deepcopy(self._last_world),
            "after": copy.deepcopy(self._last_world), "preview": copy.deepcopy(self._depot_preview),
    ...
    def live_report(self):
        ...
        depots = [record for record in self._depot_records if record["receipt"] is not None]
        per_peer_depot = {peer: any(record["request"]["peer"] == peer for record in depots) for peer in ROSTER}
        occupied = [record for record in self._depot_records if record["preview"]["reason"] == "site_occupied"]
        same_batch_sites = []
        for batch in self._batches:
            by_peer = {peer: {item["command"]["site"] for item in batch["requests"][peer]
                             if item["command"]["op"] == "BUILD_DEPOT"} for peer in ROSTER}
            for site in sorted(by_peer["a"] & by_peer["b"]):
                same_batch_sites.append({"batch_hash": batch["batch_hash"], "site": site})
        verified_conflicts = []
        for common in same_batch_sites:
            matching = [record for record in self._depot_records if record["batch_hash"] == common["batch_hash"]
                        and record["request"]["command"]["site"] == common["site"]]
            built = [record for record in matching if record["receipt"] is not None]
            rejected = [record for record in matching if record["preview"]["reason"] == "site_occupied"]
            if len(built) == 1 and any(record["request"]["peer"] != built[0]["request"]["peer"] for record in rejected):
                verified_conflicts.append(copy.deepcopy(common))
        depot_coverage = {"per_peer_successful_depot": per_peer_depot,
            "built_while_running": any(record["before"]["paused"] is False for record in depots),
            "built_while_paused": any(record["before"]["paused"] is True for record in depots),
            "occupied_site_rejections": len(occupied), "same_batch_shared_sites": same_batch_sites,
            "same_batch_conflicts_verified": verified_conflicts,
            "successful_depots": len(depots), "rejected_depots": len(self._depot_records) - len(depots),
            "observed_scope": "bounded_launcher_depot_callbacks_objects_costs_and_order"}
        depot_met = (progress["completed"] and all(per_peer_depot.values())
                     and depot_coverage["built_while_running"] and depot_coverage["built_while_paused"]
                     and bool(occupied))
        return {"id": ID, "completed": progress["completed"],
                ...
                "required_interactions_met": depot_met,
                "required_depot_interactions_met": depot_met, "depot_coverage": depot_coverage,
                "depot_preview_records": copy.deepcopy(self._depot_previews),
                "depot_records": copy.deepcopy(self._depot_records),
...
class ManualDepotCoordinator(_LiveState, Coordinator):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        _need(self._step_us == STEP_US and MANUAL_DEPOT_CAPABILITY in self._capabilities, "live capability and quantum required")
        self._init_live()
        self._operation = -1
        self._replies = {}
    ...
    def _validate(self, message):
        ...
        if kind in MANUAL_DEPOT_WORLD_RECEIPTS:
            _take_world(message)
    ...
    def _on_live_previewed(self, peer, message):
        request = self._batch_order[self._batch_cursor]
        _need(request["command"]["op"] == "BUILD_DEPOT" and _take_world(message) == self._last_world,
              "preview changed the held world or targets a non-depot command")
        validate_preview(message["preview"], request["command"], self._last_world)
        if not self._collect(peer, message, "live_previewed"):
            return []
        _need(self._replies["a"]["preview"] == self._replies["b"]["preview"], "depot preview results differ")
        self._depot_preview = copy.deepcopy(message["preview"])
        self._record_preview(request)
        if not self._depot_preview["allowed"]:
            return self._send("live_reject", "live_rejected", request=request,
                preview_digest=digest(self._depot_preview), batch_hash=self._batch_hash, **self._last_world)
        key = request["peer"] + ":" + str(self._engine_seq[request["peer"]] + 1)
        return self._send("live_apply", "live_applied", request=request, command_key=key,
                          batch_hash=self._batch_hash, **self._last_world)

    def _on_live_rejected(self, peer, message):
        _need(_take_world(message) == self._last_world and self._depot_preview is not None
              and self._depot_preview["allowed"] is False
              and message["preview_digest"] == digest(self._depot_preview),
              "depot rejection changed the held world or its agreed preview")
    ...
    def _on_live_applied(self, peer, message):
        request = self._batch_order[self._batch_cursor]
        depot = request["command"]["op"] == "BUILD_DEPOT"
        self._at_frontier(message, self.paused if depot else request["command"]["value"])
        if depot:
            key = request["peer"] + ":" + str(self._engine_seq[request["peer"]] + 1)
            validate_build_receipt(message["receipt"], request["command"], key,
                                   self._depot_preview, self._last_world, _take_world(message))
        else:
...
class ManualDepotReplica(_LiveState, Replica):
    def __init__(self, *args, live_input_source=None, stop_requested=None, stream_engine_factory=None, **kwargs):
        super().__init__(*args, **kwargs)
        _need(self.step_us == STEP_US and MANUAL_DEPOT_CAPABILITY in self.capabilities, "live capability and quantum required")
        self._init_live()
    ...
    def _do_apply(self, message):
        ...
        depot = request["command"]["op"] == "BUILD_DEPOT"
        if depot:
            _need(self._depot_preview is not None and self._depot_preview["allowed"] is True,
                  "depot command lacks its agreed successful preview")
        receipt = (self.stream_engine.apply_manual(request["command"], expected_key, self._depot_preview)
                   if depot else self.stream_engine.apply(request["command"], expected_key))
        after = self._fresh()
        if depot:
            validate_build_receipt(receipt, request["command"], expected_key, self._depot_preview, before, after)
        else:
    ...
    def _next_command(self):
        if self._batch_cursor == len(self._batch_order):
            self._next = {"live_settle"}
        else:
            command = self._batch_order[self._batch_cursor]["command"]
            self._next = {"live_preview" if command["op"] == "BUILD_DEPOT" else "live_apply"}

    def _do_preview(self, message):
        before = self._fresh()
        request = self._batch_order[self._batch_cursor]
        key = request["peer"] + ":" + str(self._engine_seq[request["peer"]] + 1)
        _need(request["command"]["op"] == "BUILD_DEPOT" and _take_world(message) == before
        ...
        preview = self.stream_engine.preview(request["command"], key)
        after = self._fresh()
        _need(after == before, "preview changed the held world")
        self._depot_preview = validate_preview(preview, request["command"], before)
        self._record_preview(request)
    ...
    def _do_reject(self, message):
        ...
        _need(request["command"]["op"] == "BUILD_DEPOT" and _take_world(message) == before
              and message["batch_hash"] == self._batch_hash and message["request"] == request
              and self._depot_preview is not None and self._depot_preview["allowed"] is False
              and message["preview_digest"] == digest(self._depot_preview),
        ...
        _need(after == before, "rejected depot changed the observed world")
        preview_hash = digest(self._depot_preview)
