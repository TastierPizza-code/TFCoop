"""T2 railway input and compared observed-engine receipt contracts."""
from __future__ import annotations

import copy

from .core import ProtocolError, canonical_json, _hash
from .rail_catalog import STEPS, get_step
...
def _need(condition, reason):
    if not condition:
        raise ProtocolError("rail input: " + reason)
...
def validate_command(command):
    ...
    if command.get("op") == "END_TEST":
        ...
        _need(set(command) == {"op", "step"} and command.get("op") == "RAIL_ACTION"
              and type(command["step"]) is int and 1 <= command["step"] <= len(STEPS),
              "RAIL_ACTION requires one known integer step")
    return copy.deepcopy(command)


def validate_preview(value, command, world):
    validate_command(command)
    _need(command["op"] == "RAIL_ACTION", "preview requires a guided action")
    ...
    expected = value["observation"].get("expected")
    _need(type(expected) is dict and expected.get("required_effect") == step["action"],
          "preview lacks the expected railway action")
...
def validate_action_receipt(value, command, command_key, preview, before, after):
    ...
    _need(type(result) is dict and set(result) == {"op", "step", "action", "command_key", "effect"}
          and result["op"] == "RAIL_ACTION" and type(result["step"]) is int
    ...
    elif step["finance"] == "credit":
        _need(effect["balance_after"] >= effect["balance_before"], "sale unexpectedly debited company money")
    elif step["finance"] == "none":
        _need(effect["balance_after"] == effect["balance_before"], "no-cost action changed company money")
    # Native proposal quotes are compared when available. Other paid commands
    # retain an explicitly observed actual debit/refund; no model price is sent
    # by the launcher or substituted for the actual company observation.
    quoted = preview["observation"].get("expected", {}).get("cost")
    if quoted is not None:
        _need(type(quoted) is int and effect["balance_before"] - effect["balance_after"] == quoted,
              "actual company delta differs from the approved native cost")
    ...
    for field in ("created", "removed"):
        ids = effect[field]
        _need(type(ids) is list and len(ids) <= 128 and all(type(key) is str and 0 < len(key) <= 128 for key in ids)
