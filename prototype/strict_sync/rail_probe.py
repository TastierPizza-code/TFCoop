...
from .rail_catalog import STEPS, get_step, catalogue
...
ID = "guided-fixed-rail-lifecycle-v1"
RAIL_CAPABILITY = "guided_rail_v1"
MAX_CYCLES = 24000
...
RAIL_WORLD_RECEIPTS = frozenset(("live_ready", "live_checkpointed", "live_applied", "live_held", "live_finished", "live_previewed", "live_rejected"))
...
def _need(condition, reason):
    if not condition:
        raise ProtocolError("rail guided probe: " + reason)
...
def schedule():
    return {"id": ID, "step_us": STEP_US, "chunk_steps": CHUNK_STEPS,
            ...
            "input_ops": ["RAIL_ACTION"], "catalogue": catalogue(),
...
class _LiveState:
    ...
    def guide_status(self):
        ...
        return {"index": self._guided_completed, "step": current["step"] if current else None,
                ...
                "scope": "fixed_launcher_rail_actions", "native_ui_input_capture": False}
    ...
    def live_report(self):
        ...
        return {"id": ID, "completed": progress["completed"],
                ...
                "coverage": {"completed_step_ids": self.guide_status()["completed_step_ids"],
                    ...
                    "observed_scope": "fixed_launcher_rail_signal_train_lifecycle",
...
class RailCoordinator(_LiveState, Coordinator):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        _need(self._step_us == STEP_US and RAIL_CAPABILITY in self._capabilities, "live capability and quantum required")
        self._init_live()
        self._operation = -1
        self._replies = {}
    ...
    def _validate(self, message):
        ...
        if kind in RAIL_WORLD_RECEIPTS:
            _take_world(message)
    ...
    def _on_live_previewed(self, peer, message):
        request = self._batch_order[self._batch_cursor]
        _need(request["command"]["op"] == "RAIL_ACTION" and _take_world(message) == self._last_world,
...
class RailReplica(_LiveState, Replica):
    def __init__(self, *args, live_input_source=None, stop_requested=None, stream_engine_factory=None, **kwargs):
        super().__init__(*args, **kwargs)
        _need(self.step_us == STEP_US and RAIL_CAPABILITY in self.capabilities, "live capability and quantum required")
        self._init_live()
    ...
    def _do_start(self, message):
        ...
        if factory is None:
            from .rail_engine import RailStreamEngine
            factory = RailStreamEngine
    ...
    def _do_preview(self, message):
        ...
        request = self._batch_order[self._batch_cursor]
        key = request["peer"] + ":" + str(self._engine_seq[request["peer"]] + 1)
        _need(request["command"]["op"] == "RAIL_ACTION" and _take_world(message) == before
