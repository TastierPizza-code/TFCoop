...
does not capture native game UI or construction.
"""
...
ID = "paced-live-input-v1"
PACED_LIVE_CAPABILITY = "paced_live_v1"
MAX_CYCLES = 2048
...
PACED_LIVE_WORLD_RECEIPTS = frozenset(("live_ready", "live_checkpointed", "live_applied", "live_held", "live_finished"))
...
def _need(condition, reason):
    if not condition:
        raise ProtocolError("paced live probe: " + reason)
...
class PacedLiveCoordinator(_LiveState, Coordinator):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        _need(self._step_us == STEP_US and PACED_LIVE_CAPABILITY in self._capabilities, "live capability and quantum required")
        self._init_live()
        self._operation = -1
        self._replies = {}
    ...
    def _validate(self, message):
        ...
        if kind in PACED_LIVE_WORLD_RECEIPTS:
            _take_world(message)
...
class PacedLiveReplica(_LiveState, Replica):
    def __init__(self, *args, live_input_source=None, stop_requested=None, stream_engine_factory=None, **kwargs):
        super().__init__(*args, **kwargs)
        _need(self.step_us == STEP_US and PACED_LIVE_CAPABILITY in self.capabilities, "live capability and quantum required")
        self._init_live()
