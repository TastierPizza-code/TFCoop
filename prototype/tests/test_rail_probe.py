...
from prototype.strict_sync.rail_probe import RailCoordinator, RailReplica, RAIL_CAPABILITY, schedule, STEP_US
...
def action(step):
    return {"op": "RAIL_ACTION", "step": step}
...
def model_preview(engine, command, *, not_ready=False):
    ...
    return {"allowed": not not_ready, "reason": "not_ready" if not_ready else "",
        ...
        "observation": {"company": {"balance": engine.world["balance"], "loan": 5000000},
                        "expected": {"model_step": step["step"], "required_effect": step["action"]}}}
...
class RailStreamModel(StreamWorldModel):
...
class RailProbeTests(unittest.TestCase):
    ...
    def pair(self, inputs=None):
        inputs = inputs or {}
        caps = tuple(sorted((*CAPABILITIES, RAIL_CAPABILITY)))
        c = RailCoordinator(EPOCH, MANIFEST, caps, step_us=STEP_US)
        ...
        replicas = {peer: RailReplica(peer, EPOCH, MANIFEST, caps, engines[peer], lambda _: [],
            frame=10, step_us=STEP_US, stream_engine_factory=RailStreamModel,
    ...
    def test_complete_catalogue_uses_actor_requests_and_identical_compared_effects(self):
        ...
        self.assertEqual(engines["a"].world["balance"], 4990000)
    ...
    def test_not_ready_is_read_only_and_can_retry_without_skipping_observation(self):
        verify = next(step["step"] for step in STEPS if step["action"] == "VERIFY_TRAIN_MOVEMENT")
    ...
    def test_command_validation_excludes_coordinates_arbitrary_actions_and_bool_step(self):
        for command in ({"op": "RAIL_ACTION", "step": True}, {"op": "RAIL_ACTION", "step": 0},
                        {"op": "RAIL_ACTION", "step": 1, "pos": [1, 2, 3]}, {"op": "SET_PAUSED", "value": True}):
