...
from prototype.strict_sync.rail_catalog import get_step


class RailSubmissionTests(unittest.TestCase):
    def setUp(self):
        self.controller = workflow.SessionController(SimpleNamespace(
            test_mode=workflow.RAIL_MODE, role='a'))
        self.writer = self.controller._input_writer = Mock()
        self.writer.submit.return_value = 1
        self.status = {'test_mode': workflow.RAIL_MODE, 'peer_started': True,
    ...
    def test_role_phase_or_command_mismatch_never_writes(self):
        ...
        for command in (get_step(2)['command'], {'op': 'GUIDED_ACTION', 'step': 1}, {'op': 'END_TEST'}, {'op': 'SET_PAUSED', 'value': True}):
            with self.assertRaises(ValueError):
                self.controller.submit_live(command)
    ...
    def test_result_fallback_keeps_exact_final_guided_progress(self):
        value = {'phase': 'completed', 'completed_steps': len(workflow.guided_steps(workflow.RAIL_MODE))}
