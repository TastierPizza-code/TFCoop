...
from prototype.strict_sync import game_runner as driver, live_input, guided_probe, runner
...
class GuidedDriverTests(unittest.IsolatedAsyncioTestCase):
    async def run_fixture(self, choose_inputs, *, fault="", duplicate_checkpoint=False, hold_final=False):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            fields = dict(epoch=epoch, timeout=15, rounds=10, profile="build_v2", delay_ms=0,
                          paced_live_probe=False, guided_probe=True, manual_depot_probe=False, live_probe=False, timing_probe=False, stream_probe=False)
            ...
            class ObservedCoordinator(guided_probe.GuidedCoordinator):
            ...
            class ClockedReplica(guided_probe.GuidedReplica):
            ...
            try:
                ...
                with patch.object(driver, "ControllerGuard"), patch.object(driver, "game_is_running", return_value=False), \
                     ...
                     patch.object(driver, "GuidedEngineAdapter", side_effect=make_engine), \
                     patch.object(driver, "GuidedReplica", ClockedReplica), \
                     patch.object(driver, "GuidedStreamEngine", side_effect=make_stream), \
                    ...
                    for peer in ("a", "b"):
                        ...
                        setup = {"game_exe": str(root / "fixture-game.exe"), "native_epoch": 7,
                                 ...
                                 "measurement_preparation": SHORT_BUILD_CONTRACT, "measurement_input_mode": "guided_suite_v1"}
    ...
    async def test_real_tcp_queues_all_catalogue_actions_and_joint_final(self):
        ...
        for peer in ("a", "b"):
            ...
            actions = [command for _, _, command in context["engines"][peer].command_calls if command["op"] == "GUIDED_ACTION"]
    ...
    async def test_divergent_engine_preview_halts_the_real_transport_before_application(self):
        ...
        for engine in context["engines"].values():
            self.assertFalse(any(command["op"] == "GUIDED_ACTION" for _, _, command in engine.command_calls))
