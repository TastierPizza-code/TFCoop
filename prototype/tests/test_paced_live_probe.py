...
from prototype.strict_sync.paced_live_probe import (
    PacedLiveCoordinator, PacedLiveReplica, PACED_LIVE_CAPABILITY, PACED_LIVE_WORLD_RECEIPTS,
...
EPOCH = "paced-live-model-epoch"
MANIFEST = "c" * 64
CAPS = tuple(sorted((*CAPABILITIES, PACED_LIVE_CAPABILITY)))
...
class PacedLiveTests(unittest.TestCase):
    def pair(self, inputs=None, **coordinator_options):
        inputs = inputs or {}
        coordinator = PacedLiveCoordinator(EPOCH, MANIFEST, CAPS, step_us=STEP_US, **coordinator_options)
        engines = {peer: BaseWorldModel() for peer in ("a", "b")}
        for engine in engines.values():
            engine.world.update(frame=10, time=200000)
            engine.observed = dict(engine.world)
        sources = {peer: ScriptedInputs(inputs.get(peer, {})) for peer in engines}
        replicas = {peer: PacedLiveReplica(peer, EPOCH, MANIFEST, CAPS, engines[peer], lambda _: [],
                    frame=10, step_us=STEP_US, stream_engine_factory=StreamWorldModel,
        ...
        coordinator.round = coordinator.frame = 10
    ...
    def complete(self, coordinator, replicas):
        ...
        for replica in replicas.values():
            ...
            self.assertIsNone(replica.receive({"kind": "complete", "epoch": EPOCH, "round": 10,
    ...
    def test_source_is_sealed_once_duplicate_ready_and_command_never_reexecutes(self):
        c, r, engines, sources = self.pair({"a": {0: [pause(True)], 1: [END]}})
        ready = c._request_inputs()
        response = r["a"].receive(ready[0][1])
        self.assertEqual(r["a"].receive(ready[0][1]), response)
        self.assertEqual(sources["a"].calls, 1)
        self.assertEqual(c.receive("a", response), [])
        self.assertEqual(c.receive("a", response), [])
        actions = c.receive("b", r["b"].receive(ready[1][1]))
    ...
    def test_empty_queue_piggyback_removes_only_poll_barriers(self):
        from prototype.tests.test_live_probe import LiveTests
        original = LiveTests()
        runs = []
        for fixture in (original, self):
            coordinator, replicas, _, sources = fixture.pair({"a": {26: [END]}})
            kinds = Counter()
            actions = coordinator._request_inputs()
            while actions:
                kinds[actions[0][1]["kind"]] += 1
                actions = fixture.exchange(coordinator, replicas, actions)
            fixture.complete(coordinator, replicas)
            runs.append((kinds, coordinator.live_report()))
            self.assertEqual(sources["a"].calls, 27)
        old, new = runs
        self.assertEqual(old[0].pop("live_poll"), 27)
        self.assertEqual(new[0]["live_poll"], 0)
        self.assertEqual(old[0], new[0])
        self.assertEqual(new[0]["live_advance"], 26)
        self.assertEqual(new[0]["live_checkpoint"], 2)
        self.assertEqual(new[1]["input_seal_cycles"], 27)
        self.assertEqual(new[1]["separate_input_poll_barriers"], 0)

    def test_missing_peer_or_invalid_piggyback_cannot_authorize_next_grant(self):
        changes = [lambda reply: reply.pop("requests"),
                   lambda reply: reply.update(cycle=True),
                   lambda reply: reply.update(cycle=0),
                   lambda reply: reply.update(requests=[{"seq": 2, "command": pause(True)}]),
                   lambda reply: reply.update(state_digest="c" * 64),
                   lambda reply: reply.update(frame=reply["frame"] + 1)]
        for change in changes:
            with self.subTest(change=change):
                c, r, _, _ = self.pair()
                advance = self.exchange(c, r, c._request_inputs())
                a = r["a"].receive(advance[0][1])
                b = r["b"].receive(advance[1][1])
                self.assertEqual(c.receive("a", a), [])
                self.assertEqual(c.frame, 10)
                change(b)
                returned = c.receive("b", b)
                self.assertTrue(c.halted)
                self.assertTrue(all(message["kind"] == "halt" for _, message in returned))
                self.assertEqual(c.frame, 10)
                self.assertEqual(r["a"].stream_engine.advances, [2])

    def test_duplicate_advance_and_hold_reuse_seal_without_consuming_queue(self):
        for first, kind in ((pause(True), "live_hold"), (None, "live_advance")):
            with self.subTest(kind=kind):
                c, r, _, sources = self.pair({"a": {0: [first] if first else [], 1: [END]}})
                actions = self.until(c, r, c._request_inputs(), kind)
                reply = r["a"].receive(actions[0][1])
                count = sources["a"].calls
                self.assertEqual(reply["cycle"], 1)
                self.assertEqual(reply["requests"][-1]["command"], END)
                self.assertEqual(r["a"].receive(actions[0][1]), reply)
                self.assertEqual(sources["a"].calls, count)
                self.assertEqual(c.receive("a", reply), [])
                self.assertEqual(c.receive("a", reply), [])
                followup = c.receive("b", r["b"].receive(actions[1][1]))
                self.assertEqual(followup[0][1]["kind"], "live_checkpoint")
                self.until(c, r, followup)
                self.complete(c, r)

    def test_request_arriving_after_local_seal_waits_for_next_joint_frontier(self):
        c, r, _, _ = self.pair()
        queue = []
        def take():
            ...
            queue.clear()
            return result
        r["a"]._live_source = take
        advance = self.exchange(c, r, c._request_inputs())
        first = r["a"].receive(advance[0][1])
        self.assertEqual(first["requests"], [])
        queue.append({"seq": 1, "command": pause(True)})
        self.assertEqual(c.receive("a", first), [])
        next_advance = c.receive("b", r["b"].receive(advance[1][1]))
        self.assertEqual(next_advance[0][1]["kind"], "live_advance")
        checkpoint = self.exchange(c, r, next_advance)
        self.assertEqual(checkpoint[0][1]["kind"], "live_checkpoint")
        self.assertEqual(checkpoint[0][1]["frame"], 14)
        # A later click is neither inserted retroactively into the old seal nor
        # applied locally. It is committed at the next identical native frontier.
        hold = self.until(c, r, checkpoint, "live_hold")
        self.assertEqual(c.live_report()["command_records"][0]["after"]["frame"], 14)
        self.assertTrue(c.paused)
        queue.append({"seq": 2, "command": END})
        self.until(c, r, hold)
        self.complete(c, r)

    def test_request_during_callback_waits_for_settlement_then_next_receipt(self):
        c, r, _, _ = self.pair({"a": {0: [pause(True)]}})
        queue = []
        ...
        r["a"]._live_source = take
        queue.append({"seq": 2, "command": END})
        settled = self.exchange(c, r, apply)
        self.assertEqual(settled[0][1]["kind"], "live_settle")
        self.assertEqual(len(queue), 1)
        hold = self.exchange(c, r, settled)
        self.assertEqual(hold[0][1]["kind"], "live_hold")
        self.assertEqual(len(queue), 1)
        self.until(c, r, hold)
        self.complete(c, r)
        self.assertEqual(c.live_progress()["acknowledged_seq"]["a"], 2)

    def test_queue_failure_after_native_ack_retains_partial_joint_frontier_and_stops(self):
        c, r, _, _ = self.pair()
        advance = self.exchange(c, r, c._request_inputs())
        def broken_queue():
            raise OSError("modeled immutable queue changed while native grant completed")
        r["a"]._live_source = broken_queue
        with self.assertRaisesRegex(ProtocolError, "queue changed"):
            r["a"].receive(advance[0][1])
        self.assertTrue(r["a"].halted)
        self.assertEqual(r["a"].stream_engine.advances, [2])
        self.assertEqual(r["a"].live_report()["last_confirmed_native_frame"], 12)
        self.assertEqual(r["a"].live_report()["last_observed_frame"], 10)
        self.assertEqual(c.frame, 10)
        self.assertEqual(c.receive("b", r["b"].receive(advance[1][1])), [])
        self.assertEqual(c.frame, 10)
        c.disconnect("a", "modeled queue failure")
        self.assertTrue(c.halted)
        with self.assertRaisesRegex(ProtocolError, "already stopped"):
            r["a"].receive(advance[0][1])

    def test_host_cannot_insert_request_into_empty_peer_seal(self):
        c, r, _, _ = self.pair({"a": {0: [pause(True)]}})
        commit = self.until(c, r, c._request_inputs(), "live_commit")
        fake = copy.deepcopy(commit[1][1])
        fake["requests"]["b"] = [{"seq": 1, "command": pause(False)}]
        fake["batch_hash"] = digest({k: fake[k] for k in
            ("cycle", "requests", "frame", "sim_time_us", "paused", "state_digest")})
        with self.assertRaisesRegex(ProtocolError, "locally sealed"):
            r["b"].receive(fake)
        self.assertEqual(r["b"].stream_engine.advances, [])

    def test_old_live_capability_and_full_build_boundary_cannot_enter_new_mode(self):
        from prototype.strict_sync.live_probe import LIVE_CAPABILITY
        with self.assertRaisesRegex(ProtocolError, "capability"):
            PacedLiveCoordinator(EPOCH, MANIFEST, tuple(sorted((*CAPABILITIES, LIVE_CAPABILITY))), step_us=STEP_US)
        c, _, _, _ = self.pair()
        c.round = c.frame = 240
        with self.assertRaisesRegex(ProtocolError, "completed unpaused build"):
            c._request_inputs()
    ...
    def test_native_only_reply_and_partial_frontier_do_not_claim_fresh_world(self):
        ...
        self.assertNotIn(reply["kind"], PACED_LIVE_WORLD_RECEIPTS)
        self.assertEqual(r["a"].live_progress()["last_observed_frame"], 10)
        self.assertEqual(r["a"].live_report()["last_confirmed_native_frame"], 12)
        self.assertEqual(c.live_report()["last_confirmed_native_frame"], 10)
        self.assertEqual(c.receive("a", reply), [])
        self.assertEqual(c.frame, 10)
        ...
        for change in (lambda m: m.update(plan_hash="d" * 64), lambda m: m.update(index=99)):
            with self.subTest(change=change):
                ...
                with self.assertRaises(ProtocolError):
                    r["a"].receive(start)
        c, r, _, _ = self.pair()
        advance = self.exchange(c, r, c._request_inputs())[0][1]
        r["a"].receive(advance)
        changed = {**advance, "frame": advance["frame"] + 1}
    ...
    def test_stop_during_native_chunk_keeps_partial_ack_frontier_separate(self):
        ...
        self.assertEqual(report["last_confirmed_native_frame"], 11)
        self.assertEqual(report["last_observed_frame"], 10)
