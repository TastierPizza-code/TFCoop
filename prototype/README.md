...
`manual_depot_probe.py` ordnet gemeinsame Eingaben. `manual_depot_engine.py` kapselt zusätzliche Vorschau, wiederholte Bindungen und Kostenprüfung. Die normale `StreamEngine` und `paced_live_v1` bleiben erhalten. Der zehnrundige Vorlauf ist ein kurzer Bereitschaftsnachweis, kein vollständiger BuildProof. Der tatsächliche Zwei-PC-Lauf hat vier Depots während Fahrt/Pause und eine kostenfreie Belegungsablehnung bestätigt: 97 identische Weltjournalzeilen und 45.835 gemeinsame Baukosten. Normale Fahrt lag bei 0,947x/0,951x, mit Bauprüfungen bei 0,904x/0,905x. Ein Konflikt in derselben Sammelrunde und Renderflüssigkeit sind damit nicht nachgewiesen. [Vollständige Auswertung](docs/ALPHA516_EVIDENCE.md).
