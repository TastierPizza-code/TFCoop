...
**Alpha5.12-Dauertest / v0.5.12 ist als Referenz für die beobachtete Testszene akzeptiert und eingefroren.** Nach dem automatischen Aufbau folgen 120 Sekunden Spielzeit mit kleinen gemeinsamen Freigaben und frischen Weltvergleichen alle zehn Sekunden. Der **Vergleichstest aus Alpha5.11** bleibt auswählbar. Paket, Quelltag, private Basis und Berichte sind gesichert.

**Echter Alpha5.12-Zwei-PC-Lauf:** Beide Messprotokolle mit 270 Einträgen sind identisch. Aufbau, 600 zusätzliche Schritte, alle zwölf Kontrollpunkte, Firmenwerte und die gemeinsame Testpause stimmen überein. Das Fahrttempo liegt bei etwa 0,962x; die Spieler akzeptieren die Fahrt mit fast unsichtbaren kurzen Zucklern. Die strenge Tempoanzeige scheitert nur an einzelnen Bestätigungsabständen knapp über 400 ms. Einzelheiten: [akzeptierte Referenz](prototype/docs/ACCEPTED_BASELINE.md).

**Nächste Etappe:** [Echte, frei ausgelöste Eingaben gemeinsam übernehmen](prototype/docs/NEXT_MILESTONE.md), beginnend mit Pause/Fortsetzen und anschließend einem begrenzten manuellen Bauauftrag. Weitere Tempooptimierung wird bis zum Funktionieren der übrigen Koop-Mechaniken zurückgestellt. Der aktuelle Release ist weiter ein automatischer Test; die nächste Etappe ist noch nicht implementiert.
...
Der Dauertest gibt jeweils zwei Schritte à 0,2 Sekunden gemeinsam frei und kontrolliert die native Spieluhr nach jedem Schritt. Nach jeweils 50 Schritten, also zehn Sekunden Spielzeit, werden frische Weltzustände verglichen. **Weltabfragen, Netzwerkbestätigungen oder ein langsamer PC können weiterhin kurze Unterbrechungen verursachen.** Die beobachtete Fahrt ist für die beiden Spieler ausreichend angenehm; Renderzeiten und allgemeiner freier Spielbetrieb sind nicht nachgewiesen.
