...
1. Beide Test, TF2 und Launcher schließen. Den vorhandenen Launcher ab Alpha5.2 normal öffnen und das Update auf **Alpha5.13-Eingabetest** abwarten. Bei erster Einrichtung [TFCoop-Windows.zip herunterladen](https://github.com/TastierPizza-code/TFCoop/releases/latest/download/TFCoop-Windows.zip), vollständig entpacken und `TF2-Coop.exe` öffnen. Git und Python sind nicht nötig.
...
3. Im Reiter **Gemeinsamer Eingabetest (experimentell)** auf beiden PCs **Pause selbst steuern** wählen. Rollen, dieselbe Host-IP und einen frischen gemeinsamen Sitzungscode festlegen. Beide **Test vorbereiten und installieren**, danach **Verbinden & Test bereitstellen**.
4. Nach der Startfreigabe TF2 über Steam starten und jeweils die neu angezeigte `TF2-Koop-Messtest-….sav` laden. **TF2 Strict Sync - gemeinsamer Eingabetest (Alpha5.13)** und **Legacy Fahrzeuge** sind bereits ausgewählt. Die zufälligen Dateinamen dürfen verschieden sein.
5. Nach 240 automatischen Aufbaurunden werden die Launcher-Tasten freigegeben. Jeder soll **Gemeinsam pausieren** und **Gemeinsam fortsetzen** auslösen und jeweils die Bestätigung abwarten. Gegenseitiges Fortsetzen ebenfalls prüfen; eine Pause mindestens 45 Sekunden halten und die gemessene lange Pause abwarten. Dann **Messung gemeinsam abschließen** und auf beide Abschlussmeldungen warten. Die normalen Pause-/Tempotasten und Bauwerkzeuge im Spiel noch nicht verwenden; Kamera bewegen ist möglich.
