...
**Alpha5.8-Bautest / v0.5.8 korrigiert das Lesen nativer Straßenanschlüsse im automatischen 240-Runden-Test.** Im echten Alpha5.7-Versuch wurden Straße, Depot und zwei Haltestellen auf beiden PCs erfolgreich gebaut. Alle zwölf protokollierten Zustände und Prüfsummen stimmen überein; beide Firmen haben danach 4.682.382 bei unverändertem Kredit. Die Simulation blieb dabei pausiert.

Beim anschließenden Verbindungsauftrag behandelte unser Leser die Straßenanschlüsse fälschlich als direkt nummerierte Liste. Beide PCs brachen vor der Ausführung dieses Bauauftrags mit demselben Lesefehler ab. Der neue Leser durchläuft die tatsächlich gelieferten Einträge und prüft weiterhin Anzahl, eindeutige gültige Kanten und Endpunkte. Die Tests bilden diese Rückgabeform ausdrücklich nach.

**Die Korrektur und der weitere Ablauf müssen im echten gemeinsamen Bautest noch bestätigt werden.** Fahrzeugkauf und Linienfahrt wurden bisher nicht erreicht. Freies gleichzeitiges Bauen, Cursor und normale gemeinsame Pause-Tasten sind nicht freigeschaltet.

## Download und Start

1. Auf beiden PCs TF2, laufenden Test und alten Launcher schließen. Den vorhandenen Launcher ab Alpha5.2 normal öffnen und das automatische Update auf **Alpha5.8-Bautest** abwarten. Bei erster Einrichtung einmal [TFCoop-Windows.zip herunterladen](https://github.com/TastierPizza-code/TFCoop/releases/latest/download/TFCoop-Windows.zip), vollständig entpacken und `TF2-Coop.exe` öffnen. Git und Python werden nicht benötigt.
...
3. Beide **Test vorbereiten und installieren**, dann **Verbinden & Test bereitstellen**. Nach der Startfreigabe TF2 starten und jeweils die neu angezeigte `TF2-Koop-Messtest-….sav` laden. Unter **OPTIONEN AUSWÄHLEN → Mods** ausschließlich **TF2 Strict Sync - automatischer Bautest (Alpha5.8)** und **Legacy Fahrzeuge** aktivieren; Diagnosemod und alte Koop-Mods ausschalten.
...
Der unveränderte Ablauf enthält drei ausdrückliche Straßenverbindungen und 211 Fortschrittsschritte, zusammen 42,2 Sekunden Enginezeit. Verbindungsbau, Fahrzeugkauf und Fahrt müssen im Spiel noch bestätigt werden. Zweck und Grenzen: [Bautest](prototype/BUILD_TEST.md), [Prüfstand](prototype/VERIFICATION.md).
