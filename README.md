...
**Alpha5.6-Bautest / v0.5.6 führt zurück zum gemeinsamen automatischen 240-Runden-Test auf zwei PCs.** Die zwei vorhandenen Alpha5.4-Soloberichte wurden geborgen und ausgewertet; eine erneute Solo-Diagnose ist dafür nicht nötig.

Der experimentelle Koop-Prototyp vergleicht gemeinsame Simulationsschritte, Bauobjekte, Firmenwerte und Fahrzeugfahrt. Alpha5.6 erfasst fehlende optionale Werte ausdrücklich, akzeptiert den dokumentierten automatischen Ladekonfigurationswert und nennt bei Lesefehlern den Feldpfad. Bei einem strikten Bauabbruch kommt automatisch ein unabhängiger, begrenzter Rohbericht der bereits gebundenen Testobjekte in den Testbericht.
...
3. Beide **Test vorbereiten und installieren**, dann **Verbinden & Test bereitstellen**. Nach der Startfreigabe TF2 starten und jeweils die neu angezeigte `TF2-Koop-Messtest-….sav` laden. Unter **OPTIONEN AUSWÄHLEN → Mods** ausschließlich **TF2 Strict Sync - automatischer Bautest (Alpha5.6)** und **Legacy Fahrzeuge** aktivieren; Diagnosemod und alte Koop-Mods ausschalten.
...
Die Rohdaten zeigen ein fehlendes `timeBuild` bei vorhandenen Industriekonstruktionen. Das macht dieses Feld zu einer plausiblen Ursache des früheren allgemeinen Zahlenfehlers, belegt aber nicht seinen Wert an der damals gebauten Straße. Die neue Fassung speichert die beobachtete Verfügbarkeit im gemeinsamen Zustandsvergleich und erfasst beim nächsten Abbruch die tatsächlichen Testobjekte. Zweck und Grenzen: [API-Diagnose](prototype/API_DIAGNOSE.md), [Bautest](prototype/BUILD_TEST.md).
