...
static void LoadConfig(const wchar_t* dllPath, const wchar_t* dataDir, Config& cfg)
{
    ...
    if (slash) *slash = 0;
    const std::wstring dirs[] = { std::wstring(dataDir), std::wstring(dir) + L"\\" };
...
static void ApplySaveRole(const std::string& inst, const char* why)
{
    ...
    // TF2 Co-op exchanges only explicitly exported save files via the companion.
    if (g_xferPort == 0) return;
    if (inst == "a") {
